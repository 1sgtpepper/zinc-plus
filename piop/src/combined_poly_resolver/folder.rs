...
pub struct ConstraintFolder<'a, F: PrimeField> {
    ...
    /// Optional tag filter: when `Some(t)`, only constraints asserted
    /// via `assert_in_ideal_typed(_, _, t)` contribute to the fold;
    /// off-tag constraints (and `assert_zero` constraints) are skipped
    /// while still advancing the alpha-power index. Used by the
...
impl<'a, F: PrimeField> ConstraintBuilder for ConstraintFolder<'a, F> {
    ...
    /// `assert_zero` constraints contribute zero to the fold for an
    /// honest prover. Under a tag filter we skip them entirely; without
    /// one we still fold them so the sumcheck operates at the right
    /// degree.
    #[inline(always)]
