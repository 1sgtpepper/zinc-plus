...
(\texttt{Int<4>}, i.e.\ $256$-bit fixed-precision integers; see
\texttt{EC\_FP\_INT\_LIMBS}~$=4$). Each cell holds the canonical
...
Although every cell is stored as an \texttt{Int<4>} integer
(\Cref{sec:cols}), no range-check or lookup constraint over the
trace columns is required. The reason is that constraints encode
$\F_p$ equalities, so non-canonical rational representatives of the
witness elements ($x = x' + kp$ for any $k$) satisfy
...
There are $13$ constraints in total: $3$ doubling constraints, $2$
mixed-addition scratch constraints, $3$ output-selection constraints
(which fold in the conditional add and the next-row chaining),
$3$ init-boundary constraints, and $2$ final-row affine-readout
constraints. All constraints carry the zero ideal,
